...
function ENT:Use(ply)
    ...
    local steamlead = sql.Query("SELECT title FROM bands_members WHERE steamid64 = " .. sql.SQLStr(steamid64)) or nil -- Сделай чтобы тут проверяло не по лидеру а по тому находиться ли он в банде вообще и в какой
    if steamlead then
